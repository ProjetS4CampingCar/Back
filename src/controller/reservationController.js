const {
  ...
  material,
  sequelize,
} = require("../db/sequelize/sequelize");
...
let validateMaterialsToReserve = async (id_material, date_start, date_end) => {
  let sql =
    "SELECT DISTINCT R_M.id_material FROM Reservation_Material R_M\
  ...
  if (ids.length > 0) {
    console.log(ids);
    throw new Error(
      "These (by id) articles are already reserved at the time choosen :" +
        ids.reduce((acc, el) => {
          return acc + " " + el.id_material.toString();
        }, "")
    );
  }
};
...
let addReservation = async (req, res) => {
  ...
  try {
    ...
    await validateMaterialsToReserve(
      req.body.id_materials.map(Number),
      req.body.start,
      req.body.end
    );
    ...
    let total_price = (
      await material.sum("price", {
        where: { id: { [Op.in]: req.body.id_materials } },
      })
    ).toFixed(2);
    ...
    let mats = await material.findAll({
      where: { id: { [Op.in]: req.body.id_materials } },
    });
...
let updateReservation = async (req, res) => {
  try {
    ...
    if ("id_materials" in req.body) {
      ...
      // array of id to add
      let add = req.body.id_materials.filter(
        (element) => !ids_old.includes(element)
      );
      ...
      let del = old.filter(
        (element) => !req.body.id_materials.includes(element.dataValues.id)
      );
      ...
      let total_price =
        reserv.dataValues.total_price + price_to_add - price_to_remove;
      total_price *= nbDayBetweenTwoDates(req.body.start, req.body.end);
